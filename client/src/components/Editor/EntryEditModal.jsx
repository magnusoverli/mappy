          ...
          <Box sx={{ borderTop: 1, borderColor: 'divider', pt: 2 }}>
            ...
            {selected.length < 2 ? (
              ...
              <Collapse in={selected.length >= 2}>
                ...
                <Box sx={{ mt: 1 }}>
                  <Button size="small" onClick={() => setPreviewOpen(v => !v)}>
                    Preview Changes
                  </Button>
                  <Collapse in={previewOpen}>
                    <Box sx={{ mt: 1 }}>
                      {generatePreview().map((p, idx) => (
                        <Typography key={idx} sx={{ fontFamily: '"JetBrains Mono", monospace', color: p.newOffset < p.oldOffset ? 'success.main' : p.newOffset > p.oldOffset ? 'warning.main' : undefined }}>
